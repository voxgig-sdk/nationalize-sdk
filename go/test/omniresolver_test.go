...
import (
	...
	omni "github.com/voxgig-sdk/nationalize-sdk/go/test/omni"
...
func omniProvider(client any) *omni.Provider {
	provider := &omni.Provider{
		...
		Errify: func(err any) any {
			if sdkerr, is := err.(*sdk.NationalizeError); is {
				out := map[string]any{
					"name":    "NationalizeError",
...
func omniCtx(arg any, client *sdk.NationalizeSDK, utility *sdk.Utility) *sdk.Context {
	...
	if provider, is := ctxmap["client"].(*omni.Provider); is {
		if live, is := defproviderclient(provider).(*sdk.NationalizeSDK); is {
			client = live
