...
class NationalizeTestRunner:
    ...
    def load_env_local():
        ...
        for line in content.splitlines():
            ...
            NationalizeTestRunner._env[key] = val

    @staticmethod
    def getenv(key):
        val = NationalizeTestRunner._env.get(key)
        if val is not None:
            return val
    ...
    @staticmethod
    def env_override(m):
        live = NationalizeTestRunner.getenv("NATIONALIZE_TEST_LIVE")
        override = NationalizeTestRunner.getenv("NATIONALIZE_TEST_OVERRIDE")

        if live == "TRUE" or override == "TRUE":
            for key in list(m.keys()):
                envval = NationalizeTestRunner.getenv(key)
        ...
        explain = NationalizeTestRunner.getenv("NATIONALIZE_TEST_EXPLAIN")
        if explain is not None and explain != "":
            m["NATIONALIZE_TEST_EXPLAIN"] = explain
    ...
    def load_test_control():
        ...
        """
        if NationalizeTestRunner._test_control is not None:
            return NationalizeTestRunner._test_control
        ctrl_path = os.path.join(os.path.dirname(__file__), "sdk-test-control.json")
        try:
            with open(ctrl_path, "r") as f:
                NationalizeTestRunner._test_control = json.load(f)
        except (FileNotFoundError, IOError, ValueError):
            NationalizeTestRunner._test_control = {
                "version": 1,
                "test": {"skip": {
    ...
    def is_control_skipped(kind, name, mode):
        ...
        ctrl = NationalizeTestRunner.load_test_control()
...
def load_env_local():
    NationalizeTestRunner.load_env_local()


def env_override(m):
    return NationalizeTestRunner.env_override(m)


def entity_data(v):
    return NationalizeTestRunner.entity_data(v)


def entity_list_to_data(lst):
    return NationalizeTestRunner.entity_list_to_data(lst)


def is_control_skipped(kind, name, mode):
    return NationalizeTestRunner.is_control_skipped(kind, name, mode)


def load_test_control():
    return NationalizeTestRunner.load_test_control()


def live_client_options():
    return NationalizeTestRunner.live_client_options()


def live_delay_ms():
    return NationalizeTestRunner.live_delay_ms()
