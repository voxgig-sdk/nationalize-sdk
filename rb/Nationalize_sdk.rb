...
require_relative 'Nationalize_types'
...
class NationalizeSDK
  ...
  def initialize(options = {})
    ...
    feature_opts = NationalizeHelpers.to_map(VoxgigStruct.getprop(@options, "feature"))
    if feature_opts
      featureorder = VoxgigStruct.getpath(@options, "__derived__.featureorder")
      if featureorder.is_a?(Array)
        featureorder.each do |fname|
          fopts = NationalizeHelpers.to_map(feature_opts[fname])
          if fopts && fopts["active"] == true
            utility.feature_add.call(@_rootctx, NationalizeFeatures.make_feature(fname))
          end
        end
      end
    end
  ...
  def get_utility
    NationalizeUtility.copy(@_utility)
  end
  ...
  def prepare(fetchargs = {})
    ...
    ctrl = NationalizeHelpers.to_map(VoxgigStruct.getprop(fetchargs, "ctrl")) || {}
    ...
    params = NationalizeHelpers.to_map(VoxgigStruct.getprop(fetchargs, "params")) || {}
    query = NationalizeHelpers.to_map(VoxgigStruct.getprop(fetchargs, "query")) || {}
  ...
  def raw_request(fetchargs = {})
    ...
    if fetched.is_a?(Hash)
      status = NationalizeHelpers.to_int(VoxgigStruct.getprop(fetched, "status"))
  ...
  def graphql(query, variables = nil, ctrl = nil)
    ...
    if errors.is_a?(Array) && !errors.empty?
      ...
      res["ok"] = false
      res["err"] = NationalizeError.new(
        "graphql_error", "NationalizeSDK: graphql: #{msg}")
      res["graphql"] = errors
    end
  ...
  def self.test(testopts = nil, sdkopts = nil)
    ...
    sdk = NationalizeSDK.new(sdkopts)
    sdk.mode = "test"
    sdk
  end
end
