# Nationalize Ruby SDK



The Ruby SDK for the Nationalize API — an entity-oriented client using idiomatic Ruby conventions.

The SDK exposes the API as capitalised, semantic **Entities** — for example `client.PredictNationality` — with named operations (`load`) instead of raw URL paths and query strings. Working with resources and verbs keeps call sites self-describing and reduces cognitive load.
...
```ruby
client = NationalizeSDK.test(testopts, sdkopts)
```
...
raise a `NationalizeError` (a `StandardError` subclass), so wrap
...
```
rb/
├── Nationalize_sdk.rb       -- Main SDK module
...
The main module (`Nationalize_sdk`) exports the SDK class
