...
module NationalizeUtilities
  MakeRequest = ->(ctx) {
    ...
    spec = ctx.spec
    utility = ctx.utility
    response = NationalizeResponse.new({})
    ...
    if fetch_err
      ...
      response = NationalizeResponse.new({ "err" => ctx.make_error("request_no_response", "response: undefined") })
    elsif fetched.is_a?(Hash)
      response = NationalizeResponse.new(fetched)
    else
