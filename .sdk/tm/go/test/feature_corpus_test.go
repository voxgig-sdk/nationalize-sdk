...
//
// NewNationalizeSDK, not TestSDK: the `test` feature is transport: 'base' and
// REPLACES the transport, so a client in test mode would shadow the script.
func fcClient(kase map[string]any) *sdk.NationalizeSDK {
	res, _ := kase["res"].([]any)
	opts := map[string]any{
	...
	return sdk.NewNationalizeSDK(opts)
}
...
func fcCandidates(client *sdk.NationalizeSDK) []fcOp {
	out := []fcOp{}
...
func fcInvoke(client *sdk.NationalizeSDK, op fcOp, ctrl map[string]any) error {
	cv := reflect.ValueOf(client)
	ct := cv.Type()
	mapType := reflect.TypeOf(map[string]any{})
...
// new go here.
func fcRecord(client *sdk.NationalizeSDK, name string) any {
