# Nationalize TypeScript SDK



The TypeScript SDK for the Nationalize API — a type-safe, entity-oriented client with full async/await support.
...
```ts
try {
  const predictnationality = await client.PredictNationality().load({ id: 'example_id' })
...
```ts
const client = new NationalizeSDK({ apikey: '...' })
...
| `tester(testopts?, sdkopts?)` | `NationalizeSDK` | Create a test-mode client instance. |

#### Static methods

| Method | Returns | Description |
| --- | --- | --- |
| `NationalizeSDK.test(testopts?, sdkopts?)` | `NationalizeSDK` | Create a test-mode client. |
...
| `client` | `client(): NationalizeSDK` | Return the parent SDK client. |
...
nationalize/
├── src/
│   ├── NationalizeSDK.ts        # Main SDK class
