...
//     (\`new NationalizeSDK(...)\` / \`NationalizeSDK.test(...)\` become
//     \`NationalizeSDK.test({ entity: ... })\`), transpiled to JS, and run
...
const TEST_SEED = {"entity":{"predict_nationality":{"test01":{"id":"test01"}}}}
...
// rewrite real client construction (\`new NationalizeSDK(...)\` and any
// \`NationalizeSDK.test(...)\`) to a seeded test-mode client, and give
...
function rewriteForRun(code: string): string {
  ...
  out = out.replace(/new\s+NationalizeSDK\s*\([^)]*\)/g, () => SEEDED_CTOR)
  out = out.replace(/NationalizeSDK\.test\s*\([^)]*\)/g, () => SEEDED_CTOR)
