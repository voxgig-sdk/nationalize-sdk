...
(0, node_test_1.describe)('PredictNationalityEntity', async () => {
    // Per-test live pacing. Delay is read from sdk-test-control.json's
    // `test.live.delayMs`; only sleeps when NATIONALIZE_TEST_LIVE=TRUE.
    (0, node_test_1.afterEach)((0, utility_1.liveDelay)('NATIONALIZE_TEST_LIVE'));
    (0, node_test_1.test)('instance', async () => {
        const testsdk = __1.NationalizeSDK.test();
    ...
    (0, node_test_1.test)('basic', async (t) => {
        ...
        for (const op of ['load']) {
            if ((0, utility_1.maybeSkipControl)(t, 'entityOp', 'predict_nationality.' + op, live))
                return;
        }
        ...
        let predict_nationality_ref01_data = Object.values(setup.data.existing.predict_nationality)[0];
        // LOAD
        const predict_nationality_ref01_ent = client.PredictNationality();
        const predict_nationality_ref01_match_dt0 = {};
        ...
        (0, node_assert_1.default)(null != predict_nationality_ref01_data_dt0);
    });
});
function basicSetup(extra) {
    ...
    const entityDataFile = node_path_1.default.resolve(__dirname, '../../../../.sdk/test/entity/predict_nationality/PredictNationalityTestData.json');
    // TODO: file ready util needed?
    const entityDataSource = Fs.readFileSync(entityDataFile).toString('utf8');
    // TODO: need a xlang JSON parse utility in voxgig/struct with better error msgs
    const entityData = JSON.parse(entityDataSource);
    options.entity = entityData.existing;
    let client = __1.NationalizeSDK.test(options, extra);
    const struct = client.utility().struct;
    const merge = struct.merge;
    const transform = struct.transform;
    let idmap = transform(['predict_nationality01', 'predict_nationality02', 'predict_nationality03'], {
        '`$PACK`': ['', {
                '`$KEY`': '`$COPY`',
    ...
    if (live) {
        client = new __1.NationalizeSDK(merge([
...
//# sourceMappingURL=PredictNationalityEntity.test.js.map