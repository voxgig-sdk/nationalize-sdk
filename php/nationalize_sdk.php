...
class NationalizeSDK
{
    ...
    public function __construct(array $options = [])
    {
        ...
        $feature_opts = NationalizeHelpers::to_map(Struct::getprop($this->options, "feature"));
        if ($feature_opts) {
            $featureorder = Struct::getpath($this->options, "__derived__.featureorder");
            if (is_array($featureorder)) {
                foreach ($featureorder as $fname) {
                    $fopts = NationalizeHelpers::to_map($feature_opts[$fname] ?? null);
                    if ($fopts && isset($fopts["active"]) && $fopts["active"] === true) {
                        ...
                        if (!NationalizeFeatures::has_feature($fname)) {
                        ...
                        ($utility->feature_add)($this->_rootctx, NationalizeFeatures::make_feature($fname));
                    }
                }
            }
        }
    ...
    {
        return NationalizeUtility::copy($this->_utility);
    }
    ...
    public function prepare(array $fetchargs = []): mixed
    {
        ...
        $ctrl = NationalizeHelpers::to_map(Struct::getprop($fetchargs, "ctrl")) ?? [];
        ...
        $params = NationalizeHelpers::to_map(Struct::getprop($fetchargs, "params")) ?? [];
        $query = NationalizeHelpers::to_map(Struct::getprop($fetchargs, "query")) ?? [];
    ...
    private function op_denied(string $op): array
    {
        ...
        return [
            "ok" => false,
            "err" => new NationalizeError($op . "_allow",
                "NationalizeSDK: " . $op . ": operation not allowed by" .
    ...
    private function raw_request(array $fetchargs = []): mixed
    {
        ...
        if (is_array($fetched)) {
            $status = NationalizeHelpers::to_int(Struct::getprop($fetched, "status"));
    ...
    public function graphql(string $query, ?array $variables = null, ?array $ctrl = null): mixed
    {
        ...
        if (is_array($errors) && 0 < count($errors)) {
            ...
            $res["ok"] = false;
            $res["err"] = new NationalizeError("graphql_error",
                "NationalizeSDK: graphql: " . $msg);
            $res["graphql"] = $errors;
        }
    ...
    public function PredictNationality($data = null)
    {
        ...
        if ($data === null) {
            if ($this->_predict_nationality === null) {
                $this->_predict_nationality = new PredictNationalityEntity($this, null);
            }
            return $this->_predict_nationality;
        }
        return new PredictNationalityEntity($this, $data);
    }
    ...
    public static function test(?array $testopts = null, ?array $sdkopts = null): self
    {
        ...
        $sdk = new NationalizeSDK($sdkopts);
        $sdk->mode = "test";
        return $sdk;
    }
}
