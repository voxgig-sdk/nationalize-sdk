...
class FtCtrl extends NationalizeControl
{
...
class FtClient
{
    ...
    private NationalizeUtility $utility;

    public function __construct(NationalizeUtility $utility, array $options)
    {
    ...
    public function get_utility(): NationalizeUtility
    {
        return $this->utility;
    }
}
...
class FtRecorder
{
    ...
    {
        return function (NationalizeContext $_ctx, string $url, array $fetchdef): array {
...
class FtHarness
{
    ...
    public NationalizeContext $rootctx;
    ...
    public function __construct(
        ...
        $utility->fetcher = $server ?? function (NationalizeContext $_ctx, string $_url, array $fetchdef): array {
        ...
        foreach ($features as $fspec) {
            ...
            $f = NationalizeConfig::make_feature($name);
    ...
    private static function buildUrl(NationalizeSpec $spec): string
    {
        $keys = [];
    ...
    public function op(array $o = []): array
    {
        ...
        if (($ctx->out['point'] ?? null) instanceof NationalizeError) {
        ...
        if (is_array($fetched)) {
            $ctx->response = new NationalizeResponse($fetched);
        }
    ...
    private function fail(NationalizeContext $ctx, mixed $err): array
    {
...
class FeatureTest extends TestCase
{
    ...
    public function test_retry_retries_a_transport_error_until_exhausted(): void
    {
        ...
        $clock = new FtClock();
        $n = 0;
        $server = function (NationalizeContext $ctx, string $_url, array $_fd) use (&$n): array {
            $n++;
