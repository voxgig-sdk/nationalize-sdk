...
boru-driven command-line client **and** interactive REPL for the Nationalize
...
```sh
# 1. Build a native binary (-> dist/<os>-<arch>/nationalize-cli)
make build

# 2. See usage (words, entities, env vars)
./nationalize-cli --help

# 3. Provide credentials once, via the environment
export NATIONALIZE_APIKEY=sk_live_xxx

# 4. Each command line is ONE boru expression, run against the API:
./nationalize-cli load 1 predict_nationality            # {id:1} shorthand
./nationalize-cli load '{id:1}' predict_nationality       # explicit match map

# 5. Override the API base URL for a single call
NATIONALIZE_BASE=https://api.example.com ./nationalize-cli load 1 predict_nationality

# 6. No arguments -> interactive REPL
./nationalize-cli
nationalize> load 1 predict_nationality
nationalize> /quit
```
...
1. **Build the binary.** From this `go-cli/` directory:
   ...
   ```sh
   make build          # -> dist/<os>-<arch>/nationalize-cli
   ```
...
3. **Run a query.** Evaluate an boru expression against the API (or run with no
   ...
   ```sh
   ./dist/*/nationalize-cli load 1 predict_nationality
   ```
...
```sh
./nationalize-cli load 1 predict_nationality          # scalar shorthand for {id:1}
./nationalize-cli load '{id:1}' predict_nationality     # explicit match map
```
...
./nationalize-cli load 1 predict_nationality
```
...
Run with no arguments to open a REPL (prompt `nationalize>`). Each line is
...
- `<entity>` is a bareword, auto-quoted as an boru atom (e.g. `predict_nationality`).
...
| Target | Result |
|--------|--------|
| `make build` | Native binary at `dist/<os>-<arch>/nationalize-cli`. |
...
(`./nationalize-cli <expr>`) and interactively (the REPL), and expressions compose the
