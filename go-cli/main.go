// Command Nationalize-cli is a CLI + REPL for the Nationalize SDK,
...
const prompt = "nationalize"

// entitiesHelp is the space-separated entity list shown by /help.
const entitiesHelp = "predict_nationality"
...
func usage(out io.Writer) {
	...
	fmt.Fprintln(out, "Env:      NATIONALIZE_APIKEY (api key), NATIONALIZE_BASE (base url override)")
...
func run(args []string, in io.Reader, out, errOut io.Writer) int {
	...
	// Configure from the environment: NATIONALIZE_APIKEY carries the API key and
	// NATIONALIZE_BASE optionally overrides the API base URL (e.g. production).
	// Both injectable by a secrets vault. Unset -> nil config defaults.
	var opts map[string]any
	if apikey := os.Getenv("NATIONALIZE_APIKEY"); apikey != "" {
		opts = map[string]any{"apikey": apikey}
	}
	if base := os.Getenv("NATIONALIZE_BASE"); base != "" {
		if opts == nil {
			opts = map[string]any{}
		}
		opts["base"] = base
	}
	client := sdk.NewNationalizeSDK(opts)
