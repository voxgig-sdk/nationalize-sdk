...
[MCP](https://modelcontextprotocol.io) server exposing the Nationalize SDK as
two agent tools — `nationalize_list` and `nationalize_load` — built on the
...
```sh
# 1. Build a native binary (-> dist/<os>-<arch>/nationalize-mcp)
make build

# 2. Provide credentials via the environment
export NATIONALIZE_APIKEY=sk_live_xxx

# 3a. Install into Claude Code over stdio (most common)
claude mcp add --scope user nationalize \
  -- /absolute/path/to/nationalize-mcp -transport stdio

# 3b. …or run a shared HTTP server instead
./nationalize-mcp -transport http -addr :8080
```

Tool-call arguments (what an agent sends):

```jsonc
// nationalize_list: first page of records
{ "entity": "predict_nationality" }
{ "entity": "predict_nationality", "query": { } }

// nationalize_load: one record by id
{ "entity": "predict_nationality", "query": { "id": 1 } }
```
...
1. **Build** the server from this `go-mcp/` directory:
   ...
   ```sh
   make build          # -> dist/<os>-<arch>/nationalize-mcp
   ```
...
3. **Install it into Claude Code** (stdio transport):
   ...
   ```sh
   claude mcp add --scope user nationalize \
     -- "$PWD"/dist/*/nationalize-mcp -transport stdio
   ```

4. **Restart Claude Code.** The `nationalize_list` and `nationalize_load` tools now appear
   in new sessions. Ask the agent to *"list predict_nationality using nationalize"*
   and it calls `nationalize_list` with `{"entity":"predict_nationality"}`.
...
```sh
export NATIONALIZE_APIKEY=sk_live_xxx            # API key
export NATIONALIZE_BASE=https://api.example.com  # optional: override the API base URL
```
...
### Call the `nationalize_list` tool
...
### Call the `nationalize_load` tool
...
| Tool | Args | Returns |
|------|------|---------|
| `nationalize_list` | `entity` (required), `query` (optional map) | First page of records as JSON |
| `nationalize_load` | `entity` (required), `query` = `{id:N}` | Single record as JSON |
...
| Variable | Purpose |
|----------|---------|
| `NATIONALIZE_APIKEY` | API key sent with every request. |
| `NATIONALIZE_BASE` | Optional override of the API base URL. |
...
```sh
./nationalize-mcp -transport http -addr :18080 &
...
curl -sN -X POST http://localhost:18080 \
  ...
  -d '{"jsonrpc":"2.0","id":2,"method":"tools/call","params":{"name":"nationalize_load","arguments":{"entity":"predict_nationality","query":{"id":1}}}}'
```
